...
import { Card, CardHeader, Avatar, CardContent, Typography, Grid, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
...
function DataDisplay() {
  ...
  useEffect(() => {
    axios.get('https://jop-api-wccz.onrender.com/jobs')
      .then((response) => {
  ...
  return (
    <Grid container spacing={2}>
      {cardsToDisplay.map((item) => (
        <Grid item xs={12} sm={6} md={4} lg={4} key={item.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <Card style={{ width: '300px', height: '263px', borderRadius: '0px 0px 0px 0px', background: '#fff' }}>
            ...
            <CardContent>
              ...
              <Typography style={{ color: '#000', fontFamily: 'Roboto', fontSize: '17px', fontWeight: '400', lineHeight: 'normal' }}>{item.title}</Typography>
              <Typography style={{ color: '#000', fontFamily: 'Roboto', fontSize: '17px', fontWeight: '600', lineHeight: 'normal' }}>Vacancy Available:</Typography>
              <Typography style={{ color: '#000', fontFamily: 'Roboto', fontSize: '17px', fontWeight: '400', lineHeight: 'normal' }}>{item.availability}</Typography>
      ...
      {selectedCard && (
        <Dialog open={!!selectedCard} onClose={closePopup} maxWidth="md" fullWidth>
          ...
          <DialogContent>
            <Typography>Position: {selectedCard.title}</Typography>
            <Typography>Vacancy: {selectedCard.availability}</Typography>
        ...
        <DialogContent>
          ...
          {selectedCard && (
            <div>
              ...
              <Typography>Name: {selectedCard.title}</Typography>
              <Typography>Position: {selectedCard.category}</Typography>
            </div>
          )}
        </DialogContent>
        <DialogActions>
